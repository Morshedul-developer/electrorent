...
export const AddItemView: React.FC<AddItemViewProps> = ({
    ...
    return (
      <div className="max-w-md mx-auto my-16 p-8 bg-surface-card rounded-2xl border border-border-main text-center space-y-4">
        ...
        <p className="text-xs text-text-muted">
          Only authorized administrators with chief logistics clearance can add
          items to the ElectroRent fleet.
        </p>
        <button
  ...
  const [location, setLocation] = useState(
    "Central Warehouse Stage 4B, Austin",
  );
      ...
      switch (fieldName) {
        ...
        case "serialNumber":
          if (!val.trim()) {
            ...
            copy.serialNumber =
              "Serial number must start with prefix 'SN-' (e.g. SN-DJI-1234).";
          } else if (val.trim().length < 6) {
            copy.serialNumber =
              "Serial number must be at least 6 characters long.";
          } else {
        ...
        case "imageUrl":
          if (val && !/^https?:\/\/.+/i.test(val)) {
            copy.imageUrl =
              "Please input a valid URL starting with http:// or https://";
          } else {
        ...
        case "description":
          if (val && val.length < 20) {
            copy.description =
              "Detailed technical summary must be at least 20 characters.";
          } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Run absolute final cross-validation
    ...
    if (Object.keys(finalErrors).length > 0) {
      setFormError(
        "Please correct all validation errors highlighted in the requisition form.",
      );
      onShowToast("Requisition payload contains invalid credentials.", "error");
      return;
    }
    ...
    try {
      ...
      onShowToast(
        `Smart gadget '${title}' successfully added to fleet list!`,
        "success",
      );
      onNavigate("explore");
    } catch (err: any) {
      setFormError(err.message || "Failed to commit item.");
      onShowToast(err.message || "Requisition failed.", "error");
    } finally {
      setSubmitting(false);
    }
  };
        ...
        <div className="flex items-center gap-3 border-b border-border-main pb-4 mb-6">
          ...
          <div>
            <h1 className="font-display text-xl font-bold text-text-main">
              Requisition New Fleet Asset
            </h1>
            <p className="text-xs text-text-dim mt-0.5">
              Surgically register certified smart hardware in central database
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div className="space-y-1.5 sm:col-span-2">
              <label className="text-[10px] uppercase font-mono tracking-wider text-text-dim">
                Visual Asset Image URL (Unsplash recommended)
              </label>
              <input
                type="url"
            ...
            <div className="space-y-1.5 sm:col-span-2">
              ...
              <input
                ...
                onBlur={() =>
                  validateField("shortDescription", shortDescription)
                }
                className={`w-full px-3.5 py-2.5 bg-bg-dark rounded-lg border text-text-main text-xs focus:outline-none focus:ring-1 placeholder-text-dim ${
            ...
            <div className="space-y-1.5 sm:col-span-2">
              <label className="text-[10px] uppercase font-mono tracking-wider text-text-dim">
                Detailed Description (Spec Sheets)
              </label>
              <textarea
          ...
          <div className="border-t border-border-main/50 pt-6 flex justify-end gap-4">
            <button
              type="button"
              onClick={() => onNavigate("manage-inventory")}
              className="px-6 py-2.5 rounded-lg bg-surface-bright text-text-main text-xs font-bold transition-colors"
            >
              Cancel Setup
            </button>
            <button
              type="submit"
              disabled={submitting}
              className="px-6 py-2.5 rounded-lg bg-brand-primary hover:bg-brand-primary/95 text-text-main text-xs font-bold transition-colors cursor-pointer"
            >
              {submitting
                ? "Writing telemetry payload..."
                : "Commit Fleet Requisition"}
            </button>
          </div>
        </form>
      </div>
